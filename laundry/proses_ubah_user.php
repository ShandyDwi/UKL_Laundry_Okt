...
if($_POST){
    ...
    if(empty($nama)){
        ...
        include "koneksi.php";
        if(empty($password)){
            $update=mysqli_query($conn,"update user set nama='".$nama."', role='".$role."', username='".$username."', where id='".$id."'") or die(mysqli_error($conn));
            if($update){
            ...
            $update=mysqli_query($conn,"update user set nama='".$nama."', role='".$role."', username='".$username."', password='".md5($password)."' where id='".$id."'") or die(mysqli_error($conn));
            if($update){
                echo "<script>alert('Sukses update user');location.href='tampil_user.php';</script>";
            } else {
                echo "<script>alert('Gagal update user');location.href='ubah_user.php?id=".$id."';</script>";
